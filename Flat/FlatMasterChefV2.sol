...
contract EggToken is BEP20('Turkey', 'TRK') {
...
contract MasterChefV2 is  ReentrancyGuard {
    ...
    // Presale end?
    bool public preSaleEnd = false;
    mapping (address => uint256) public preSaleAmount;
    uint256 public preSaleRatio = 1000;
    uint256 public totalPreSaleAmount = 100000000000000000000;
    uint256 public assetConvertUint256 = 1000000000000000000;
    ...
    constructor(
     ...
     function burnToken(uint256 _amount) public {
        require(msg.sender == devaddr,"only dev can burn token");
        safeEggTransfer(address(0),_amount);
    ...
    function airDrop(address[] memory _recipients, uint256  _amount) public {
        ...
        require(msg.sender == devaddr, "only dev can airdrop");
        for (uint256 i = 0; i < _recipients.length; i++) {
            safeEggTransfer(_recipients[i],_amount);
        }
    }
    ...
    function airDropWithDifferentAmount(address[] memory _recipients, uint256[] memory _amounts) public {
        ...
        require(msg.sender == devaddr,"only dev can airdop with different amount");
        for (uint256 i = 0;i < _recipients.length; i++) {
            safeEggTransfer(_recipients[i],_amounts[i]);
    }
    }

    function adjustPreSaleAmount(uint256 _adjustAmount) public {
        require(msg.sender == devaddr,"only dev can adjust presale");
        totalPreSaleAmount = _adjustAmount;
    }

    

    function adjustPreSaleRatio(uint256 _adjustRatio) public 
    {
        require(msg.sender == devaddr,"only dev can adjust presale ratio");
        preSaleRatio = _adjustRatio;
    }

     receive() external payable {
        uint256 buyAmount = msg.value * preSaleRatio;
        require(!preSaleEnd && totalPreSaleAmount > buyAmount && msg.value > 0, "presale end or sold out");
        totalPreSaleAmount -= buyAmount;
        safeEggTransfer(msg.sender,buyAmount);
    }

     function withdrawlICO(uint256 _amount) public payable {
        require(msg.sender == devaddr,"only dev can withdraw");
        msg.sender.transfer(_amount);

    }

    function makeIcoEnd() public {
        require(msg.sender == devaddr,"Only dev can make ICO end");
        preSaleEnd = true;
    }

    function icoEnd() public returns (bool)
    {
        return preSaleEnd;
    }
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public  nonDuplicated(_lpToken) {
        require(msg.sender == devaddr,"owner function");
         // 1% = 100 _allocPointForDev
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public  {
        require(msg.sender == devaddr,"owner function");
    ...
    function updateEmissionRate(uint256 _eggPerBlock) public {
        require(msg.sender == devaddr,"owner function");
        massUpdatePools();
        eggPerBlock = _eggPerBlock;
