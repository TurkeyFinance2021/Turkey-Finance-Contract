...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        TURKEY.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        TURKEY.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeTURKEYTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeTURKEYTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe TURKEY transfer function, just in case if rounding error causes pool to not have enough TURKEYs.
    function safeTURKEYTransfer(address _to, uint256 _amount) internal {
        uint256 TURKEYBal = TURKEY.balanceOf(address(this));
        if (_amount > TURKEYBal) {
            TURKEY.transfer(_to, TURKEYBal);
        } else {
            TURKEY.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        TURKEY.safeTransfer(address(msg.sender), _amount);
